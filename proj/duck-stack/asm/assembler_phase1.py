...
class AsmSrcKind(Enum):
    ...
    DATA = auto()
    # Instruction using label instead of reg_src1, reg_src2, offset
    MEMOP = auto()
...
# Pattern for JUMP pseudo-instruction
...
def transform(lines: list[str]) -> list[str]:
    ...
    for lnum in range(len(lines)):
        ...
        try:
            ...
            if fields["kind"] == AsmSrcKind.MEMOP:
                # replace with a FULL instruction line
                fix_optional_fields(fields)
                # using PC-relative address
                ref = fields["labelref"]
                mem_addr = labels[ref]
            ...
            # Increment address if not comment
            if fields["kind"] != AsmSrcKind.COMMENT:
                address += 1
        # Relay error messages to user if any occur
...
def resolve(lines: list[str]) -> dict[str, int]:
    """
    Build table associating labels and addresses
    """
    labels: dict[str, int] = {}
    address = 0
    for lnum in range(len(lines)):
        # eliminate whitespaces ruining parsing
        line = lines[lnum].rstrip()
        log.debug(f"Resolving line {lnum}: {line}")
        try:
            fields = parse_line(line)
            ...
            # Errors will be handled in transorm()
    return labels
